const awsServerlessExpress = require('../index')
const lambdaFunction = require('../example/lambda.js')
const apiGatewayEvent = require('../example/api-gateway-event.json')

function clone(json) {
  return JSON.parse(JSON.stringify(json))
}

function makeEvent(eventOverrides) {
  const baseEvent = clone(apiGatewayEvent)

  return Object.assign({}, baseEvent, eventOverrides)
}

function expectedRootResponse() {
  return makeResponse({
    "body": "",
    "headers": {
      "content-encoding": "gzip",
      "content-type": "text/html; charset=utf-8",
      "etag": "W/\"ea3-WawLnWdlaCO/ODv9DBVcX0ZTchw\""
    }
  })
}

function makeResponse(response) {
  const baseResponse = {
    "body": "",
    "isBase64Encoded": true,
    "statusCode": 200
  }
  const baseHeaders = {
    "access-control-allow-origin": "*",
    "connection": "close",
    "content-type": "application/json; charset=utf-8",
    "vary": "Accept-Encoding",
    "x-powered-by": "Express"
  }
  const headers = Object.assign({}, baseHeaders, response.headers)
  const finalResponse = Object.assign({}, baseResponse, response)
  finalResponse.headers = headers
  return finalResponse
}

describe('e2e', () => {
  // test('GET HTML (initial request)', (done) => {
  //   const succeed = response => {
  //     delete response.headers.date
  //     expect(response).toEqual(expectedRootResponse())
  //     done()
  //   }
  //   const server = lambdaFunction.handler(makeEvent({ path: '/', httpMethod: 'GET' }), {
  //     succeed
  //   })
  // })

  // test('GET HTML (subsequent request)', (done) => {
  //   const succeed = response => {
  //     delete response.headers.date
  //     expect(response).toEqual(expectedRootResponse())
  //     done()
  //   }
  //   const server = lambdaFunction.handler(makeEvent({
  //     path: '/',
  //     httpMethod: 'GET'
  //   }), {
  //     succeed
  //   })
  // })

  test('GET JSON collection', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "W3siaWQiOjEsIm5hbWUiOiJKb2UifSx7ImlkIjoyLCJuYW1lIjoiSmFuZSJ9XQ==",
        "headers": {
          "content-length": "46",
          "etag": "W/\"2e-Lu6qxFOQSPFulDAGUFiiK6QgREo\"",
        }
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users',
      httpMethod: 'GET'
    }), {
      succeed
    })
  })

  test('GET missing route', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "PCFET0NUWVBFIGh0bWw+CjxodG1sIGxhbmc9ImVuIj4KPGhlYWQ+CjxtZXRhIGNoYXJzZXQ9InV0Zi04Ij4KPHRpdGxlPkVycm9yPC90aXRsZT4KPC9oZWFkPgo8Ym9keT4KPHByZT5DYW5ub3QgR0VUIC9ub3RoaW5nLWhlcmU8L3ByZT4KPC9ib2R5Pgo8L2h0bWw+Cg==",
        "headers": {
          "content-length": "151",
          "content-security-policy": "default-src \'self\'",
          "content-type": "text/html; charset=utf-8",
          "x-content-type-options": "nosniff",
        },
        statusCode: 404
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/nothing-here',
      httpMethod: 'GET'
    }), {
      succeed
    })
  })

  test('GET JSON single', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "eyJpZCI6MSwibmFtZSI6IkpvZSJ9",
        "headers": {
          "content-length": "21",
          "etag": "W/\"15-rRboW+j/yFKqYqV6yklp53+fANQ\"",
        }
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users/1',
      httpMethod: 'GET'
    }), {
      succeed
    })
  })

  test('GET JSON single 404', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "e30=",
        "headers": {
          "content-length": "2",
          "etag": "W/\"2-vyGp6PvFo4RvsFtPoIWeCReyIC8\"",
        },
        statusCode: 404
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users/3',
      httpMethod: 'GET'
    }), {
      succeed
    })
  })

  test('success - image response', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "",
        "headers": {
          "accept-ranges": "bytes",
          "cache-control": "public, max-age=0",
          "content-length": "15933",
          "content-type": "image/png",
          "etag": "W/\"3e3d-15d9e97dd38\"",
          "last-modified": "Tue, 01 Aug 2017 16:18:59 GMT",
          "vary": undefined
        }
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/sam',
      httpMethod: 'GET'
    }), {
      succeed
    })
  })

  test('POST JSON', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "eyJpZCI6MywibmFtZSI6IlNhbmR5In0=",
        "headers": {
          "content-length": "23",
          "etag": "W/\"17-oE5Vx0BytQgD+icR9TBb/jl0xzw\"",
        },
        statusCode: 201
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users',
      httpMethod: 'POST',
      body: '{"name": "Sandy"}'
    }), {
      succeed
    })
  })

  test('GET JSON single (again; post-creation) 200', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "eyJpZCI6MywibmFtZSI6IlNhbmR5In0=",
        "headers": {
          "content-length": "23",
          "etag": "W/\"17-oE5Vx0BytQgD+icR9TBb/jl0xzw\"",
        },
        statusCode: 200
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users/3',
      httpMethod: 'GET'
    }), {
      succeed
    })
  })

  test('DELETE JSON', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "W3siaWQiOjIsIm5hbWUiOiJKYW5lIn0seyJpZCI6MywibmFtZSI6IlNhbmR5In1d",
        "headers": {
          "content-length": "48",
          "etag": "W/\"30-qo1gXFWFPSt69BQXQRRCuLoQPUg\"",
        },
        statusCode: 200
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users/1',
      httpMethod: 'DELETE'
    }), {
      succeed
    })
  })

  test('forwardConnectionErrorResponseToApiGateway', (done) => {
    // NOTE: the fact that including a body in this GET request causes this to fail is a bug
    // See https://github.com/awslabs/aws-serverless-express/issues/106
    // abusing this bug for now to get forwardConnectionErrorResponseToApiGateway coverage
    // but this test will need to change when this gets fixed
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual({
        "body": "",
        "headers": {},
        statusCode: 502
      })
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/',
      httpMethod: 'GET',
      body: "{\"name\": \"Sam\"}"
    }), {
      succeed
    })
  })

  test('PUT JSON', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "eyJpZCI6MiwibmFtZSI6IlNhbXVlbCJ9",
        "headers": {
          "content-length": "24",
          "etag": "W/\"18-uGyzhJdtXqacOe9WRxtXSNjIk5Q\"",
        },
        statusCode: 200
      }))
      done()
    }
    const server = lambdaFunction.handler(makeEvent({
      path: '/users/2',
      httpMethod: 'PUT',
      body: '{"name": "Samuel"}'
    }), {
      succeed
    })
  })

  test('base64 encoded request', (done) => {
    const succeed = response => {
      delete response.headers.date
      expect(response).toEqual(makeResponse({
        "body": "eyJpZCI6MiwibmFtZSI6IlNhbXVlbCJ9",
        "headers": {
          "content-length": "24",
          "etag": "W/\"18-uGyzhJdtXqacOe9WRxtXSNjIk5Q\"",
        },
        statusCode: 200
      }))
      done()
    }
    lambdaFunction.handler(makeEvent({
      path: '/users/2',
      httpMethod: 'PUT',
      body: btoa('{"name": "Samuel"}'),
      isBase64Encoded: true
    }), {
      succeed
    })
  })

  test('server.onClose', (done) => {
    const succeed = response => {
      server.on('close', () => {
        expect(server._isListening).toBe(false)
        done()
      })
      server.close()
    }
    const server = lambdaFunction.handler(makeEvent({}), {
      succeed
    })
  })

  describe('error cases', () => {
    let server
    test('forwardLibraryErrorResponseToApiGateway; serverListenCallback; no binaryTypes', (done) => {
      const awsServerlessExpress2 = require('../index')
      const app = function (req, res) {
        res.end('')
      }
      const serverListenCallback = jest.fn()
      server = awsServerlessExpress2.createServer(app, serverListenCallback)
      const succeed = response => {
        expect(response).toEqual({
          statusCode: 500,
          body: '',
          headers: {}
        })
        expect(serverListenCallback).toHaveBeenCalled()
        done()
      }
      awsServerlessExpress.proxy(server, null, {
        succeed
      })
    })

    test('server.onError EADDRINUSE', (done) => {
      const awsServerlessExpress2 = require('../index')
      const app = function (req, res) {
        res.end('')
      }
      const server2 = awsServerlessExpress2.createServer(app)
      server2._socketPathSuffix = server._socketPathSuffix
      const succeed = response => {
        expect(response.statusCode).toBe(200)
        done()
        server.close()
        server2.close()
      }
      awsServerlessExpress.proxy(server2, makeEvent({}), {
        succeed
      })
    })
  })
})